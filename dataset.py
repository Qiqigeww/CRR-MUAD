...
class RealIADDataset(torch.utils.data.Dataset):
    def __init__(self, root, category, transform, gt_transform, phase, anomaly_source_path=None, image_size=448):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'train':
            ...
            img, augmented_image, anomaly_mask, label = self.transform_image(img, img_path)
            ...
            name = f'{self.classname}_{img_type}_{os.path.basename(img_path[:-4])}'
            
            return img_ori, augmented_image, anomaly_mask #, label, name
        ...
        if label == 0:
            ...
            ##############################################
            gt = torch.where(
                    gt < 0.5, torch.zeros_like(gt), torch.ones_like(gt)
                )
            ##############################################
        assert img.size()[1:] == gt.size()[1:], "image.size != gt.size !!!"
    ...
    def transform_image(self, image, img_path):
        image = np.array(image)
        image = cv2.resize(image, dsize=(self.image_size, self.image_size))
        
        cv2_image=image
        thresh_path = self.get_foreground(img_path)
        thresh=cv2.imread(thresh_path,0)
        thresh = cv2.resize(thresh,dsize=(self.image_size, self.image_size))
        ...
        augmented_image, anomaly_mask, has_anomaly  = self.perlin_synthetic(image, thresh, anomaly_path, cv2_image)
    ...
    def get_foreground(self,image_path):
        foreground_path = image_path.replace('/realiad_1024/', '/realiad_1024_DISthresh/')
        foreground_path = foreground_path.replace('/OK/', '/DISthresh/')
        if not os.path.exists(foreground_path):
            raise FileNotFoundError(f"The path {foreground_path} does not exist.")
    ...
    def perlin_synthetic(self, image, thresh, anomaly_source_path, cv2_image):

        no_anomaly = torch.rand(1).numpy()[0]
        if no_anomaly > 1.0:#0.5 生成正常样本30%
            image = image.astype(np.float32)
            return image, np.zeros((self.image_size, self.image_size, 1), dtype=np.float32), np.array([0.0], dtype=np.float32)

        else:
            perlin_scale = 6  

            min_perlin_scale = 0 #2
            ...
            while(has_anomaly == 0 and try_cnt<50):  
                ...
                perlin_noise = rand_perlin_2d_np(
                    (self.image_size, self.image_size), (perlin_scalex, perlin_scaley))
                ...
                ##############################################
                # object_perlin = self.filter_largest_connected_component(object_perlin)
                ##############################################
                ...
                if np.sum(msk) !=0: 
                    has_anomaly = 1        
                try_cnt+=1
            if self.classname in texture_list:#没有纹理类别的！
    ...
    def filter_largest_connected_component(self, mask):
        num_labels, labels_im = cv2.connectedComponents(mask.astype(np.uint8))
        if num_labels <= 1:
            return mask  # 只有背景或一个连通区域

        # 计算每个连通区域的像素数
        area = [np.sum(labels_im == i) for i in range(1, num_labels)]

        # 找到最大的连通区域
        largest_index = np.argmax(area) + 1  # +1 因为标签0是背景

        # 创建新的掩膜，只包含最大的区域
        filtered_mask = np.zeros_like(mask)
        filtered_mask[labels_im == largest_index] = 1
