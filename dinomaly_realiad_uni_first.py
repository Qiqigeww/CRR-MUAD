# This script is intended to perform the decoder training in the first stage.
...
from utils import evaluation_batch, global_cosine_hm_percent, regional_cosine_focal, \
    regional_cosine_hm, WarmCosineScheduler, evaluation_batch_adeval
...
def train(item_list):
    ...
    for epoch in range(int(np.ceil(total_iters / len(train_dataloader)))):
        ...
        for (img, aug_img, aug_mask) in train_dataloader:
            ...
            en, de = model(img)
            ##############################################
            en_aug, de_aug = model(aug_img)
            ##############################################
            # loss = global_cosine(en, de)

            p_final = 0.9
            p = min(p_final * it / 1000, p_final)
            loss_normal = global_cosine_hm_percent(en, de, p=p, factor=0.1)
            loss_anormal = global_cosine_hm_percent(en, de_aug, p=p, factor=0.1)
            loss = loss_normal + loss_anormal
            ...
            if (it + 1) % 50000 == 0:
                ...
                for item, test_data in zip(item_list, test_data_list):
                    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=False,
                                                                  num_workers=4)
                    results = evaluation_batch_adeval(model, test_dataloader, device, max_ratio=0.01, resize_mask=256)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str,
                        default='vitill_realiad_uni_dinov2_3')
    parser.add_argument("--anomaly_source_path", type=str, default='/mnt/disk2/xxxx/Datasets/dtd', help='path to save results') 
