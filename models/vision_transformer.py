...
class bMlp6(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 grad=1.0, lambda_mgd=0.2):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Linear(in_features, hidden_features)
    ...
    def forward(self, x):
        B, N, C = x.shape
        mat = torch.rand((B, N, C)).to(x.device)
        mat = torch.where(mat > 1 - self.lambda_mgd, 0, 1).to(x.device)
        masked_x = torch.mul(x, mat)
        x = self.drop(masked_x)
        x = self.fc1(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
...
class DecoderReconstructive(nn.Module):
    def __init__(self, base_width, out_channels=1, output_size=(256, 256)):
        ...
        self.up4 = nn.Sequential(nn.Upsample(size=output_size, mode='bilinear', align_corners=True),
        ...
        self.db4 = nn.Sequential(
            ...
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width, 48, kernel_size=3, padding=1),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True)
        )

        self.fin_out = nn.Sequential(nn.Conv2d(48, out_channels, kernel_size=3, padding=1))
